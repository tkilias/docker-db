...
"$PIPENV" run ./exadt collect-info --outfile "$INFO_FILE" MyCluster
"$PIPENV" run ./exadt exec -c "/bin/bash -c 'X=\$(ls /usr/opt/EXASuite-*/EXASolution-*/bin/Console/exaplus | tail -n1); echo \"SELECT 123*42345;\" | \$X -c n11:8563 -u sys -P exasol -jdbcparam validateservercertificate=0'" MyCluster 2>&1 | tee /dev/stderr | grep -q 5208435
"$PIPENV" run ./exadt list-dbs MyCluster
